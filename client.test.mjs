...
// 11a. the numbers
{
  ...
  check('the numbers: the median tile says what a median is', await api.evaluate(() => /Half of all posts score under/.test(document.querySelector('#panel-metrics .mx-tile:nth-child(3)').textContent)));
  check('the numbers: combos are listed as pairs, with both definitions on hover, on their own denominator', await api.evaluate(() => { const li = document.querySelector('#mx-combos li'); const tip = JSON.parse(li.getAttribute('data-tip')); return /Excited to announce \+ A wall of hashtags/.test(li.textContent) && /Opens with/.test(tip[1]) && /And: /.test(tip[1]) && /250 of 500/.test(tip[2]); }));
