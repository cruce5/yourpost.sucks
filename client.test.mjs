...
// 8a2. what's new: the header link, the once-only pop-up, the meaner bar
{
  ...
  await api.waitForTimeout(500);
  check('  ...and the pirate flag crossfades to the red one', await api.evaluate(() => { const f = document.getElementById('pirateflag'); const red = f.querySelector('.flag-red'), black = f.querySelector('.flag-black'); return getComputedStyle(red).opacity === '1' && getComputedStyle(black).opacity === '0' && red.naturalWidth > 0 && f.getAttribute('aria-label') === 'Pirate mode'; }));
...
// 11a. the numbers
{
  const figures = (scored) => ({ ok: true, live: true, at: '2026-09-21T20:00:00.000Z', since: '2026-09-21', scored, clean: Math.round(scored * 0.1),
    ...
    meaner: { n: Math.round(scored * 0.22), of: scored, since: '2026-09-22' },
    pairs: { of: scored, since: '2026-09-22', top: [{ a: 'announce', b: 'hashwall', n: Math.round(scored / 2) }] },
  ...
  check('the numbers: every headline states a finding, written from the figures', /^60% of posts barely suck$/.test(seen.heads[0]) && /^The most popular sin is "Excited to announce", in 44% of posts$/.test(seen.heads[1]) && /^When Reword runs, it beats the original 71% of the time$/.test(seen.heads[2]) && /^80% of AI calls finish in under ten seconds$/.test(seen.heads[3]) && /^25% of posts came with an image or video$/.test(seen.heads[4]), JSON.stringify(seen.heads));
  ...
  check('the numbers: every mark has a tip, the tip opens on hover and on focus, and it names the check, its definition and its figure', await api.evaluate(() => { const p = document.getElementById('panel-metrics'); const li = p.querySelector('#mx-rules li'); li.dispatchEvent(new MouseEvent('mousemove', { bubbles: true, clientX: 200, clientY: 300 })); const tip = document.getElementById('mx-tip'); const hover = tip.classList.contains('on') && /Excited to announce/.test(tip.textContent) && /Anywhere counts/.test(tip.textContent) && /44%/.test(tip.textContent); p.dispatchEvent(new MouseEvent('mouseleave')); const gone = !tip.classList.contains('on'); li.focus(); const focused = tip.classList.contains('on'); return hover && gone && focused && p.querySelectorAll('[data-tip]').length > 40 && [...p.querySelectorAll('svg rect[data-tip], .mx-stack span[data-tip]')].every(e => e.getAttribute('tabindex') === '0'); }));
  ...
  check('the numbers: the median tile says what a median is', await api.evaluate(() => /The middle post scores \d\.something\. \d+% score under \d\./.test(document.querySelector('#panel-metrics .mx-tile:nth-child(3)').textContent)));
